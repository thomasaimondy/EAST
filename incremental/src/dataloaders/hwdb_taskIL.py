...
def get(seed=0, mini = False):
    ...
    for i in range(labsize//subclass):
        data[i] = {}
        data[i]['name'] = 'hwdbCIL-{:d}'.format(i)
        data[i]['ncla'] = labsize//5
        for s in ['train','test']:
            data[i][s] = {'x': [], 'y': []}
    for s in ['train', 'test']:
        ...
        for image, target in loader:
            ...
            index = np.where(target.numpy()[0]==seeds)[0][0] // subclass
            subindex = np.where(target.numpy()[0]==seeds)[0][0] % subclass
            # Separate different samples into different tasks
            if not subindex:
                data[index][s]['x'].insert(0, image)
                data[index][s]['y'].insert(0, target.numpy()[0])
            else:

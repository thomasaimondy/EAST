...
def get(seed=0, fixed_order=False, pc_valid=0, nclass=10,args=None):
    nclass=10
    data = {}
    taskcla = []
    size = [1, 32, 32]
    ...
    # if not os.path.isdir('../dat/binary_hwdb_'+str(nclass)+'/'):
    #     os.makedirs('../dat/binary_hwdb_'+str(nclass))
    if True:
        # Pre-load
        # MNIST
        mean = (0,)
        ...
        dat['train'] = HWDB(root+'/train_mat.txt', num_class=nclass, incremental=False, transforms=transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean, std)]))
        dat['test'] = HWDB(root+'/test_mat.txt', num_class=nclass, incremental=False, transforms=transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean, std)]))
        for i, r in enumerate(seeds):
            ...
            for s in ['train', 'test']:
                ...
                # torch.save(data[i][s]['x'],os.path.join(os.path.expanduser('../dat/binary_hwdb_'+str(nclass)+'/'), 'data' + str(r) + s + 'x.bin'))
                # torch.save(data[i][s]['y'],os.path.join(os.path.expanduser('../dat/binary_hwdb_'+str(nclass)+'/'), 'data' + str(r) + s + 'y.bin'))
        print()
    ...
    #         for s in ['train', 'test']:
    #             data[i][s] = {'x': [], 'y': []}
    #             data[i][s]['x'] = torch.load(os.path.join(os.path.expanduser('../dat/binary_hwdb_'+str(nclass)), 'data' + str(r) + s + 'x.bin'))
    #             data[i][s]['y'] = torch.load(os.path.join(os.path.expanduser('../dat/binary_hwdb_'+str(nclass)), 'data' + str(r) + s + 'y.bin'))

...
def get(seed=0, mini = False):
    ...
    print(seeds.shape, seeds[:10])

    # if not os.path.isdir('../dat/binary_hwdbCIL_{}/'.format(seed)):
    #     os.makedirs('../dat/binary_hwdbCIL_{}'.format(seed))
    dat = {}
    # root = '/home/user/liuhongxing/BRP-SNN-origin/DATASETS/HWDB'
    root = '/home/user/jiashuncheng/E-Flow/dat/HWDB'
    dat['train'] = HWDB(root+'/train_mat.txt', num_class=labsize-1, transforms=None)
    dat['test'] = HWDB(root+'/test_mat.txt', num_class=labsize-1, transforms=None)
    print('load...')
    for i, r in enumerate(seeds):
        data[i] = {}
        data[i]['name'] = 'hwdbCIL-{:d}'.format(r)
        data[i]['ncla'] = labsize
        for s in ['train', 'test']:
            data[i][s] = {'x': [], 'y': []}
    for s in ['train', 'test']:
    ...
    for i, r in enumerate(seeds):
        for s in ['train', 'test']:
            ...
            # torch.save(data[i][s]['x'],os.path.join(os.path.expanduser('../dat/binary_hwdbCIL_{}/'.format(seed)), 'data' + str(r) + s + 'x.bin'))
            # torch.save(data[i][s]['y'],os.path.join(os.path.expanduser('../dat/binary_hwdbCIL_{}/'.format(seed)), 'data' + str(r) + s + 'y.bin'))
    # else:
    #     for i, r in enumerate(seeds):
    ...
    #         for s in ['train', 'test']:
    #             data[i][s] = {'x': [], 'y': []}
    #             data[i][s]['x'] = torch.load(os.path.join(os.path.expanduser('../dat/binary_hwdbCIL_{}'.format(seed)), 'data' + str(r) + s + 'x.bin'))
    #             data[i][s]['y'] = torch.load(os.path.join(os.path.expanduser('../dat/binary_hwdbCIL_{}'.format(seed)), 'data' + str(r) + s + 'y.bin'))

...
class Net(torch.nn.Module):
    ...
    def __init__(self,args,inputsize,taskcla,nlayers=2,nhid=2000,pdrop1=0.2,pdrop2=0.5):
        ...
        if not args.multi_output:
            self.last= torch.nn.Linear(nhid,self.taskcla[0][1]) # 单头
        else:
            self.last=torch.nn.ModuleList()
            for t,n in self.taskcla:
                self.last.append(torch.nn.Linear(nhid,n)) # 多头
